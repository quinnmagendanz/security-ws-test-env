...
# Two wrapper functions to allow application code to create new
# variables. They will be added to the current global current contrete
# values.
def mk_int(id):
  global current_concrete_values
  return current_concrete_values.mk_int(id)

def mk_str(id):
  global current_concrete_values
  return current_concrete_values.mk_str(id)
...
class InputQueue(object):
  def __init__(self):
    ...
    self.inputs.put((0, ConcreteValues()))
